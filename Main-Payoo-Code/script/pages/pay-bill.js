...
import { showUndo } from "../enhancements.js";
...
class PayBillController {
  constructor() {
    ...
    // Get PIN from user session (stored during login)
  ...
  async handlePayBill() {
    ...
    const billName = this.categories[this.selectedCategory].name;
    if (!window.confirm(`Confirm ${billName} payment of ৳${amount.toFixed(2)} for account ${accountNumber}?`)) {
      return;
    }
  ...
  deleteSavedBill(billId) {
    const removedBill = this.savedBills.find((bill) => bill.id === billId);
    this.savedBills = this.savedBills.filter(b => b.id !== billId);
    localStorage.setItem("saved_bills", JSON.stringify(this.savedBills));
    this.renderSavedBills();
    showUndo("Saved bill removed.", () => {
      if (!removedBill || this.savedBills.some((bill) => bill.id === removedBill.id)) return;
      this.savedBills.push(removedBill);
      localStorage.setItem("saved_bills", JSON.stringify(this.savedBills));
      this.renderSavedBills();
      displaySuccess("Saved bill restored.");
    });
  }

  deleteReminder(reminderId) {
    const removedReminder = this.reminders.find((reminder) => reminder.id === reminderId);
    this.reminders = this.reminders.filter(r => r.id !== reminderId);
    localStorage.setItem("bill_reminders", JSON.stringify(this.reminders));
    this.renderReminders();
    showUndo("Bill reminder removed.", () => {
      if (!removedReminder || this.reminders.some((reminder) => reminder.id === removedReminder.id)) return;
      this.reminders.push(removedReminder);
      localStorage.setItem("bill_reminders", JSON.stringify(this.reminders));
      this.renderReminders();
      displaySuccess("Reminder restored.");
    });
  }
  ...
  renderSavedBills() {
    ...
    container.innerHTML = this.savedBills.map(bill => `
      <div class="saved-bill flex items-center justify-between p-3 bg-base-200 rounded-xl group" data-bill-id="${bill.id}">
